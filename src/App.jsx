...
function App() {
  ...
  // Login view states: 'login' | 'signup' | 'forgot' | 'reset_passcode' | 'otp_verify'
  const [loginView, setLoginView] = useState('login');
  const [forgotEmail, setForgotEmail] = useState('');
  const [resetOtpInput, setResetOtpInput] = useState('');
  const [expectedResetOtpCode, setExpectedResetOtpCode] = useState('');
  const [useSecurityQuestionFallback, setUseSecurityQuestionFallback] = useState(false);
  ...
  const handleForgotPasscodeClick = () => {
    ...
    setUseSecurityQuestionFallback(false);
  };
  ...
  const handleForgotEmailSubmit = async (e) => {
    ...
    // 1. Online OTP request via Vercel /api/auth
    try {
      const response = await fetch('/api/auth', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          setSuccessMsg("6-Digit password reset code sent directly to your email!");
          setLoading(false);
          return;
        } else {
          setLoginError(result.error || "No account found with this email.");
          setLoading(false);
          return;
        }
      }
    } catch (err) {
      console.warn("Failed online reset OTP request, falling back to mock mode:", err);
    }

    // 2. Local fallback lookup / mock mode
    const mockCode = Math.floor(100000 + Math.random() * 900000).toString();
    setExpectedResetOtpCode(mockCode);
    setRecoveryEmail(targetEmail);
    const matchedUser = localUsers.find(usr => usr.email.trim().toLowerCase() === targetEmail);
    if (matchedUser) {
    ...
    setUseSecurityQuestionFallback(false);
    setLoginView('reset_passcode');
    setSuccessMsg(`[Mock Mode] Reset code sent to ${targetEmail}. Use code: ${mockCode}`);
    setLoading(false);
  };
  ...
  const handleResendResetOtp = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        if (result.success) {
          setSuccessMsg("A new 6-digit verification code was sent to your email!");
          setLoading(false);
          return;
        }
      }
    } catch (err) {
      console.warn("Resend reset OTP error:", err);
    }
    const mockCode = Math.floor(100000 + Math.random() * 900000).toString();
    setExpectedResetOtpCode(mockCode);
    setSuccessMsg(`[Mock Mode] New verification code: ${mockCode}`);
    setLoading(false);
  };
  ...
  const handleResetPasscodeSubmit = async (e) => {
    ...
    // If using Security Question fallback
    if (useSecurityQuestionFallback) {
      ...
      if (!securityAnswerInput || (storedAns && securityAnswerInput.trim().toLowerCase() !== storedAns.trim().toLowerCase())) {
        setLoginError('Incorrect security answer.');
        return;
      }
    } else {
    ...
    // 1. Online reset via Vercel /api/auth
    try {
      const response = await fetch('/api/auth', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          setLoginError(result.error || "Failed to reset passcode. Please check verification code.");
          setLoading(false);
          return;
        }
      }
    } catch (err) {
      console.warn("Online reset failed, trying local fallback:", err);
    }

    // 2. Local fallback verification
    if (useSecurityQuestionFallback || (expectedResetOtpCode && resetOtpInput.trim() === expectedResetOtpCode)) {
      const updatedUsers = localUsers.map(usr => {
        if (usr.email.trim().toLowerCase() === recoveryEmail) {
          return { ...usr, passcode: newPasscode };
        }
      ...
      {isAppLoading ? (
        ...
        <div className="login-overlay">
          ...
          <div className="login-card">
            ...
            {(loginView === 'forgot' || loginView === 'reset_passcode') && (
              <div className="recovery-steps-wrapper">
                <div className="recovery-steps-timeline">
                  <div className={`step-node ${loginView === 'forgot' || loginView === 'reset_passcode' ? 'active' : ''} ${loginView === 'reset_passcode' ? 'completed' : ''}`}>
            ...
            <h2 className="login-title">
              {loginView === 'login' && "IEEE Dibrugarh University Financial Portal"}
              {loginView === 'signup' && "Register Account"}
              {loginView === 'forgot' && "Forgot Passcode"}
              {loginView === 'reset_passcode' && "Reset Passcode"}
              {loginView === 'otp_verify' && "Account Authorization"}
            </h2>
            ...
            <p className="login-subtitle">
              ...
              {loginView === 'forgot' && "Enter your registered email to receive a 6-digit reset code"}
              {loginView === 'reset_passcode' && `Enter the 6-digit code sent to ${recoveryEmail} and set your new passcode`}
              {loginView === 'otp_verify' && "Enter the 6-digit authorization code dispatched to the Host"}
            </p>
            ...
            {loginView === 'login' ? (
              ...
              <form onSubmit={handleResetPasscodeSubmit} className="login-form">
                <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 'var(--radius-sm)', padding: '10px 12px', border: '1px solid rgba(255,255,255,0.08)', marginBottom: '14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', overflow: 'hidden' }}>
                    <Mail size={14} style={{ color: 'var(--primary-color, #00629B)', flexShrink: 0 }} />
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-light, #ffffff)', fontWeight: 500, textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
                      {recoveryEmail}
                    </span>
                  </div>
                  <button
                ...
                {!useSecurityQuestionFallback ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <div className="recovery-question-card">
                      ...
                      <p className="recovery-question-text">{securityQuestion || "What was the name of your first IEEE event?"}</p>
                    </div>
                    <div className="login-input-wrapper">
                      <input
                        type="text"
                ...
                <button type="submit" className="login-btn" style={{ marginTop: '14px' }} disabled={loading}>
                  {loading ? "Updating Passcode..." : "Reset Passcode & Sign In"}
                </button>

                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '12px', fontSize: '0.75rem' }}>
                  {!useSecurityQuestionFallback ? (
                    <button
                      type="button"
                      onClick={handleResendResetOtp}
                      style={{ background: 'none', border: 'none', color: 'var(--primary-light, #008080)', cursor: 'pointer', padding: 0 }}
                      disabled={loading}
                    >
                  ...
                  <button
                    type="button"
                    onClick={() => setUseSecurityQuestionFallback(!useSecurityQuestionFallback)}
                    style={{ background: 'none', border: 'none', color: 'var(--text-muted, #94a3b8)', cursor: 'pointer', padding: 0, textDecoration: 'underline' }}
                  >
                    {useSecurityQuestionFallback ? "Use Email OTP Code" : "Or use Security Question"}
                  </button>
                </div>

                <button
                  type="button"
                  onClick={() => { setLoginView('login'); setLoginError(''); setResetOtpInput(''); setNewPasscode(''); setConfirmPasscode(''); }}
                  className="login-btn btn-outline"
